...
class MeanSquareLoss(Loss):
    def forward(self, X, Y):
        ...
        """
        # calculating loss
        sum1 = np.sum((X - Y) ** 2, axis=1, keepdims=True)
        mse_loss = np.mean(sum1)
        return mse_loss
...
class MeanAbsoluteError(Function):
    def forward(self, y_ture, y_pred):
        ...
        #Forward Pass 
        sum1 = np.sum(numpy.abslute(y_true - y_pred), axis=1, keepdims=True)
        mae_loss = np.mean(sum1)
        return mae_loss

    def local_grad(self, X, Y):
        N = y_true.shape[0]
        
        grads = {"X": - ((y_true - y_pred) / (abs(y_true - y_pred) + 10**-100))/N}
        return grads
